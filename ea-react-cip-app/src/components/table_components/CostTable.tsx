...
const CostTable: React.FC<CostTableProps> = ({
  ...
  const startDurationOptions = Array.from(
    { length: 15 },
    (_, index) => 0 + index
  );
            ...
            <div key={packageName}>
              <input
                ...
                checked={selectedPackages.has(
                  numToStringTypeConverter(packageName)
                )}
                onChange={(e) =>
                  handlePackageCheckChange(
                    numToStringTypeConverter(packageName),
                    e.target.checked
                  )
                }
  ...
  const handlePackageCheckChange = (
    packageName: string,
    isChecked: boolean
  ) => {
    ...
    if (rowIndex !== null) {
      ...
      for (let i = 1; i <= 3; i++) {
        if (rowIndex + i < updatedData.length) {
          const updatedPackageSplit = stringToNumTypeConverter(
            updatedData[rowIndex + i][4]
          );
          updatedData[rowIndex + i][5] =
            (totalPackageCost * updatedPackageSplit) / 100;
          const updatedPackageCost = updatedData[rowIndex + i][5];
          const updatedRow = shiftRowDataForDuration(
            updatedData[rowIndex + i],
            updatedPackageCost,
            updatedData[rowIndex + i][3],
            updatedData[rowIndex + i][2]
          );
  ...
  const handleYearChange = (
    ...
    while (
      headerRowIndex < updatedData.length &&
      updatedData[headerRowIndex][0] !== currentGroupName
    ) {
      headerRowIndex++;
    }
      ...
      <select
        ...
        onChange={(e) =>
          handleDurationChange(
            rowIndex,
            colIndex,
            stringToNumTypeConverter(e.target.value)
          )
        }
      >
  ...
  const handleCostSplitChange = (
    ...
    while (
      ...
      data[headerRowIndex][0] !== currentGroupName
    ) {
      headerRowIndex++;
    }
  ...
  const updateTotalCosts = (rowIndex: number, updatedData: CostTableData) => {
    ...
    for (let colIndex = 6; colIndex < totalRow.length; colIndex++) {
      const feasibilityValue =
        stringToNumTypeConverter(feasibilityRow[colIndex]) || 0;
      const designValue = stringToNumTypeConverter(designRow[colIndex]) || 0;
      const constructionValue =
        stringToNumTypeConverter(constructionRow[colIndex]) || 0;
  ...
  const renderCellContent = (
    ...
    if (colIndex === 2 && !isGroupHeader(row)) {
      return renderYearDropdown(
        rowIndex,
        colIndex,
        stringToNumTypeConverter(cell)
      );
    } else if (colIndex === 3 && !isGroupHeader(row)) {
      return renderDurationDropdown(
        rowIndex,
        colIndex,
        stringToNumTypeConverter(cell)
      );
    } else if (colIndex === 4 && !isGroupHeader(row)) {
      return rendercostSplitSlider(
        rowIndex,
        colIndex,
        stringToNumTypeConverter(cell)
      );
