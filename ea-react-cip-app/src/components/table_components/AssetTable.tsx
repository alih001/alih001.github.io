...
const AssetTable: React.FC<AssetTableProps> = ({
    ...
    data.forEach((row: AssetTableRow, rowIndex: number) => {
      if (rowIndex > 0) {
        ...
        dropdownColumnIndexes.forEach((columnIndex: number) => {
          ...
          const value =
            dropdownValueMap[row[columnIndex]] ?? Number(row[columnIndex]);
  ...
  const handleDropdownChange = (
    ...
    if (dropdownColumnIndexes.includes(columnIndex)) {
      ...
      if (category1ScoreColumnIndex !== -1) {
        updatedData[rowIndex][category1ScoreColumnIndex] =
          category1Score.toFixed(2);
      }
      if (category2ScoreColumnIndex !== -1) {
        updatedData[rowIndex][category2ScoreColumnIndex] =
          category2Score.toFixed(2);
      }
      // Check if we need to update Total Score
      if (totalScoreColumnIndex !== -1) {
        updatedData[rowIndex][totalScoreColumnIndex] = (
          category1Score + category2Score
        ).toFixed(2);
      }
    }
  ...
  const renderDropdown = (rowIndex: number, columnIndex: number) => {
    ...
    return (
      <select
        value={value}
        onChange={(e) =>
          handleDropdownChange(rowIndex, columnIndex, e.target.value)
        }
      >
  ...
  const renderCollapsibleRow = (
    row: AssetTableRow,
    rowIndex: number,
    collapsibleColumnIndexes: number[]
  ) => {
    return (
      <>
        ...
        {collapsibleColumnIndexes.map((columnIndex) => (
          <StyledTableRow key={`${rowIndex}-collapsible-${columnIndex}`}>
            <StyledTableCell
              className={`${tableId}-${rowIndex}-${columnIndex}-placeholder`}
            ></StyledTableCell>
            <StyledTableCell
              className={`${tableId}-${rowIndex}-${columnIndex}-title`}
            >
              {data[0][columnIndex]}
            </StyledTableCell>
            <StyledTableCell
              className={`${tableId}-${rowIndex}-${columnIndex}-value`}
            >
              {dropdownColumnIndexes.includes(columnIndex)
                ? renderDropdown(rowIndex, columnIndex)
                : row[columnIndex]}
            </StyledTableCell>
            <StyledTableCell
              className={`${tableId}-${rowIndex}-${columnIndex}-output`}
            >
              {outputValues[`${tableId}-${rowIndex}-${columnIndex}-output`] ||
                dropdownValueMap[data[rowIndex][columnIndex]]}
            </StyledTableCell>
          </StyledTableRow>
        ))}
      </>
                  ...
                  <StyledTableRow>
                    ...
                    <StyledTableCell>
                      <Button onClick={() => toggleRowCollapse(rowIndex)}>
                        {collapsedAssetRows.has(rowIndex)
                          ? "Expand"
                          : "Collapse"}
                      </Button>
                    </StyledTableCell>
